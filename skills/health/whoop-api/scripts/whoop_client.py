...
class WhoopClient:
    ...
    def _refresh_if_needed(self) -> None:
        ...
        if not client_id or not client_secret:
            sys.exit("ERROR: WHOOP_CLIENT_ID and WHOOP_CLIENT_SECRET required for token refresh.")
    ...
    def save_data(self, data: dict[str, list[dict]], date: str | None = None) -> Path:
        ...
        date_str = date or (datetime.utcnow() - __import__("datetime").timedelta(days=1)).strftime("%Y-%m-%d")
